...
export default function QuestionsPage() {
  ...
  const openCreate = () => {
    setEditItem(null);
    setForm({ text: '', answerTemplateId: templates.filter(t => t.isActive)[0]?.id || 0, isActive: true });
          ...
          <div onMouseEnter={cancelHide} onMouseLeave={scheduleHide} style={{
            ...
            <div style={{ background: '#fff7ed', borderBottom: '1px solid #fed7aa', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#9a3412' }}><strong>{usedSurveys.length} ankette</strong> kullanılıyor</div>
              </div>
            </div>
            ...
            <div style={{ padding: '10px 14px', borderTop: '1px solid #e5e7eb', background: '#f9fafb' }}>
              <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>💡 Silmek için bu soruyu kullanan anketleri güncelleyin.</div>
              <Link to={`/admin/surveys?search=${encodeURIComponent(question.text.substring(0, 30))}`}
                ...
                onClick={() => setShow(false)}>🔗 Anketleri görüntüle →</Link>
            </div>
          </div>
        )}
      </>
    ...
    <div className="page">
      ...
      <div className="card">
        <div className="card-toolbar">
          <SearchInput value={search} placeholder="Soru metni, şablon adı, seçenek veya durum ara..."
                      ...
                      <tr key={`err-${q.id}`}>
                        <td colSpan={5} style={{ padding: 0, border: 'none' }}>
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', background: '#fff7ed', borderLeft: '4px solid #f97316', borderBottom: '1px solid #fed7aa', padding: '12px 16px', animation: 'slideDown 0.2s ease' }}>
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ fontWeight: 600, color: '#c2410c', fontSize: '13px', marginBottom: '3px' }}>Soru Silinemez — Ankette Kullanımda</div>
                              <div style={{ fontSize: '13px', color: '#374151', marginBottom: '4px' }}>Bu soru <strong>{deleteError.count} ankette</strong> kullanılmaktadır.</div>
                              <div style={{ fontSize: '12px', color: '#6b7280' }}>{deleteError.detail}</div>
                              <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '6px' }}>💡 Silmek için önce bu soruyu kullanan anketleri güncelleyin.</div>
                            </div>
                  ...
                  <div style={{
                    ...
                    <div>
                      <div style={{ fontWeight: 600, fontSize: '13px', marginBottom: '2px', color: errorType === 'duplicate' ? '#c2410c' : errorType === 'passive_conflict' ? '#854d0e' : '#dc2626' }}>
                        {errorType === 'duplicate' ? 'Yinelenen Soru Metni' : errorType === 'passive_conflict' ? 'Pasife Alınamaz — Aktif Anketlerde Kullanımda' : 'Hata'}
                      </div>
                      ...
                      {errorType === 'passive_conflict' && (<div style={{ fontSize: '12px', color: '#92400e', fontWeight: 500, marginTop: '6px' }}>💡 Bu anketleri önce pasife alın, ardından soruyu pasife alabilirsiniz.</div>)}
                      {errorType === 'duplicate' && (<div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>Soru metnini değiştirerek tekrar deneyebilirsiniz.</div>)}
                    </div>
                  </div>
                )}
                ...
                <div className="form-group">
                  ...
                  <select value={form.answerTemplateId} onChange={e => setForm(f => ({ ...f, answerTemplateId: Number(e.target.value) }))}>
                    ...
                    {templates.filter(t => t.isActive).map(t => (
                      <option key={t.id} value={t.id}>{t.name} ({t.options.length} seçenek)</option>
                    ))}
                  </select>
                </div>
                <div className="form-group">
