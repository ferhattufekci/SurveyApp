...
export default function AnswerTemplatesPage() {
  ...
  const openCreate = () => { setEditItem(null); setForm({ name: '', options: ['', ''], isActive: true }); setError(''); setShowModal(true); };
  ...
  const updateOption = (i: number, v: string) => setForm(f => ({ ...f, options: f.options.map((o, idx) => idx === i ? v : o) }));
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await answerTemplatesApi.update(editItem.id, { name: form.name, isActive: form.isActive, options: form.options.map((text, i) => ({ id: editItem.options[i]?.id ?? null, text, orderIndex: i })) });
      } else {
      ...
      const isPC  = parts.length === 3 && (msg.toLowerCase().includes('aktif soruda') || msg.toLowerCase().includes('active question'));
      setError(isPC ? parts[0] : msg);
      setErrorType(isPC ? 'passive_conflict' : isDup ? 'duplicate' : 'general');
  ...
  const handleDelete = async (id: number, rowNum: number) => {
    ...
    try { await answerTemplatesApi.delete(id); load(); showSuccess(`${rowNum} ${tx(language, t.answerTemplates.successDelete)}`); }
    catch (e: any) {
  ...
  const filtered = templates.filter(tpl => {
    const ok = activeFilter === 'active' ? tpl.isActive : activeFilter === 'passive' ? !tpl.isActive : true;
    if (!ok) return false;
    if (!search) return true;
    const q = search.toLowerCase();
    const st = tpl.isActive ? `${tx(language, t.common.active).toLowerCase()} active` : `${tx(language, t.common.passive).toLowerCase()} passive`;
    return tpl.name.toLowerCase().includes(q) || st.includes(q) || tpl.options.some(o => o.text.toLowerCase().includes(q));
  });
        ...
        {show && (
          <div onMouseEnter={keep} onMouseLeave={hide} style={{ position: 'fixed', top: pos.top - 10, left: pos.left - 310, width: '300px', background: '#fff', borderRadius: '10px', zIndex: 9999, boxShadow: '0 8px 32px rgba(0,0,0,.2)', border: '1px solid #e5e7eb', overflow: 'hidden', transform: 'translateY(-100%)' }}>
            ...
            <div style={{ maxHeight: '180px', overflowY: 'auto', padding: '8px 0' }}>
              {usedQuestions.map((q, i) => (
                <div key={q.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 14px', borderBottom: i < usedQuestions.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                  ...
                  <span style={{ flex: 1, fontSize: '12px', color: '#374151', lineHeight: 1.3 }}>{q.text.length > 48 ? q.text.substring(0, 48) + '…' : q.text}</span>
                  <span className={`badge ${q.isActive ? 'badge-success' : 'badge-secondary'}`} style={{ fontSize: '10px' }}>{q.isActive ? tx(language, t.common.active) : tx(language, t.common.passive)}</span>
                </div>
              ))}
            </div>
            <div style={{ padding: '10px 14px', borderTop: '1px solid #e5e7eb', background: '#f9fafb' }}>
              ...
              <Link to={`/admin/questions?search=${encodeURIComponent(template.name)}`} style={{ fontSize: '12px', fontWeight: 600, color: '#6366f1', textDecoration: 'none' }} onClick={() => setShow(false)}>{tx(language, t.answerTemplates.viewQuestions)}</Link>
            </div>
          </div>
        )}
      </>
    ...
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
        <div style={{ background: '#eef2ff', border: '1px solid #6366f122', borderRadius: '12px', padding: '16px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px', flexWrap: 'wrap' }}>
            ...
            <span style={totalStyle} onClick={handleTotalClick}>{tx(language, t.answerTemplates.totalTemplates)}</span>
      ...
      <div className="card">
        <div className="card-toolbar">
          <SearchInput value={search} placeholder={tx(language, t.answerTemplates.searchPlaceholder)} onChange={v => { setSearch(v); setActiveFilter('all'); setPage(1); }} />
        ...
        <div className="table-container">
          <table className="table">
            <thead><tr><th>#</th><th>{tx(language, t.common.status)}</th><th>{tx(language, t.answerTemplates.colName)}</th><th>{tx(language, t.answerTemplates.colOptionCount)}</th><th>{tx(language, t.answerTemplates.colOptions)}</th><th>{tx(language, t.common.actions)}</th></tr></thead>
            <tbody>
              {paginated.map((tpl, i) => {
                ...
                return (
                  <>
                    ...
                    {deleteError?.id === tpl.id && (
                      <tr key={`err-${tpl.id}`}><td colSpan={6} style={{ padding: 0, border: 'none' }}>
                        <div style={{ display: 'flex', gap: '12px', background: '#fff7ed', borderLeft: '4px solid #f97316', padding: '12px 16px' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: '12px', color: '#9ca3af', marginTop: '4px' }}>{tx(language, t.answerTemplates.errHint)}</div>
                          </div>
        ...
        {totalPages > 1 && (
          <div className="pagination-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderTop: '1px solid #e5e7eb', flexWrap: 'wrap', gap: '8px' }}>
            <span style={{ fontSize: '13px', color: '#6b7280' }}>{filtered.length} {language === 'tr' ? 'şablondan' : 'templates'} {(safePage - 1) * PAGE_SIZE + 1}–{Math.min(safePage * PAGE_SIZE, filtered.length)} {tx(language, t.common.showing)}</span>
          ...
          <div className={`modal${shake ? ' modal-shake' : ''}`} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                <label>{tx(language, t.answerTemplates.templateName)}<Req /></label>
                <input value={form.name} onChange={e => { setForm(f => ({ ...f, name: e.target.value })); if (errorType === 'duplicate') { setError(''); setErrorType(''); } }} placeholder={tx(language, t.answerTemplates.templateNamePh)} style={errorType === 'duplicate' ? { borderColor: '#f97316', boxShadow: '0 0 0 3px #fed7aa66' } : {}} />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                <label>{tx(language, t.answerTemplates.options)} ({form.options.length}/4)<Req /></label>
