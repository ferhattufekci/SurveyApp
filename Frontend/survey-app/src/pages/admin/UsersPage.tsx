  ...
  return (
    <div className="page">
      <style>{`
        ...
        .survey-check-item:last-child { border-bottom: none; }
        .survey-check-item:hover { background: var(--gray-50); }
        .survey-check-item input[type="checkbox"] { flex-shrink: 0; width: 16px; height: 16px; cursor: pointer; }
        .survey-check-item .check-label {
          flex: 1;
          min-width: 0;
          font-size: 13px;
          color: var(--gray-800);
        ...
        @media (max-width: 600px) {
          .survey-check-item .check-badge { max-width: 90px; }
          .survey-check-item .check-email { display: none; }
        }
      `}</style>
      ...
      <div className="card">
        ...
        <div className="table-container">
          <table className="table">
            <thead>
              <tr>
                <th>#</th><th>{tx(language, t.common.status)}</th><th>{tx(language, t.surveys.colTitle)}</th>
                <th>{tx(language, t.surveys.colQuestionsTemplates)}</th><th>{tx(language, t.surveys.colStart)}</th>
                <th>{tx(language, t.surveys.colEnd)}</th><th>{tx(language, t.surveys.colAssigned)}</th>
                <th>{tx(language, t.surveys.colResponse)}</th><th>{tx(language, t.common.actions)}</th>
              </tr>
            </thead>
            <tbody>
              {paginated.map((s, i) => {
                            ...
                            return (
                              <div key={qId} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '7px', padding: '6px 9px' }}>
                                <div style={{ fontSize: '12px', color: '#1e293b', fontWeight: 500, marginBottom: '4px', display: 'flex', gap: '5px' }}>
                                  ...
                                  <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{q.text.length > 45 ? q.text.substring(0, 45) + '…' : q.text}</span>
                                </div>
                                {tpl && (
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className={`modal modal-lg${shake ? ' modal-shake' : ''}`} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {/* ── Açıklama — zorunlu alan, * eklendi (fix #1 & #2) ── */}
              <div className="form-group">
              ...
              {/* ── Sorular — fix #3 & #4: survey-check-item ile düzgün layout ── */}
              <div className="form-group">
                <label>
                  {tx(language, t.surveys.questionsLabel)} ({form.questionIds.length} {tx(language, t.surveys.selected)})<Req />
                  {' '}<span style={{ fontSize: '12px', color: '#9ca3af', fontWeight: 400 }}>{tx(language, t.surveys.questionsNote)}</span>
                </label>
                <div className="check-scroll-list">
                  {questions.filter(q => q.isActive).length === 0 ? (
                    <div style={{ padding: '12px 14px', fontSize: '13px', color: '#9ca3af', textAlign: 'center' }}>
                      {language === 'tr' ? 'Aktif soru bulunamadı.' : 'No active questions found.'}
                    </div>
                  ) : questions.filter(q => q.isActive).map(q => (
                    <label key={q.id} className="survey-check-item" style={{ cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form.questionIds.includes(q.id)}
                        onChange={() => toggleQ(q.id)}
                      />
                      <span className="check-label" title={q.text}>{q.text}</span>
                      <span className="check-badge" title={q.answerTemplateName}>{q.answerTemplateName}</span>
                    </label>
                  ))}
                </div>
              </div>

              {/* ── Kullanıcılar — fix #3 & #4: survey-check-item ile düzgün layout ── */}
              <div className="form-group">
                <label>
                  {tx(language, t.surveys.usersLabel)} ({form.userIds.length} {tx(language, t.surveys.selected)})
                  {' '}<span style={{ fontSize: '12px', color: '#9ca3af', fontWeight: 400 }}>{tx(language, t.surveys.usersNote)}</span>
                </label>
                <div className="check-scroll-list">
                  {users.filter(u => u.isActive).length === 0 ? (
                    <div style={{ padding: '12px 14px', fontSize: '13px', color: '#9ca3af', textAlign: 'center' }}>
                      {language === 'tr' ? 'Aktif kullanıcı bulunamadı.' : 'No active users found.'}
                    </div>
                  ) : users.filter(u => u.isActive).map(u => (
                    <label key={u.id} className="survey-check-item" style={{ cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form.userIds.includes(u.id)}
                        onChange={() => toggleU(u.id)}
                      />
                      <span className="check-label" title={u.fullName}>{u.fullName}</span>
                      <span className="check-email" title={u.email}>{u.email}</span>
                    </label>
                  ))}
                </div>
              </div>
