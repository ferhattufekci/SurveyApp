...
public record AnswerOptionUpdateDto(
    int? Id,
    [Required][StringLength(200, MinimumLength = 1)] string Text,
    int OrderIndex);
...
public record SurveyDetailDto(int Id, string Title, string Description, DateTime StartDate, DateTime EndDate, bool IsActive, List<SurveyQuestionDto> Questions, List<int> AssignedUserIds);
...
// ── Report ────────────────────────────────────────────────────────────────────
public record SurveyReportDto(int SurveyId, string Title, int TotalAssigned, int TotalCompleted, int TotalPending, List<UserResponseDto> CompletedResponses, List<UserDto> PendingUsers);
